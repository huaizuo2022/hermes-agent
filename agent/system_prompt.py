...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if "memory" in agent.valid_tool_names:
        if getattr(agent, "platform", "") == "savana":
            tool_guidance.append(SAVANA_MEMORY_GUIDANCE)
        else:
    ...
    now = _hermes_now()
    # Date-only + coarse time period (Morning/Afternoon/Evening/Late Night) so the prompt is byte-stable
    # for several hours.  Minute-precision changes invalidate prefix-cache KV
    # on every rebuild path.  The model can still query the exact wall-clock time when needed.
    period = get_hermes_time_period(now)
    timestamp_line = f"Conversation started: {now.strftime('%A, %B %d, %Y')} {period}"
