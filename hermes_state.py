...
# which would otherwise fail on legacy DBs ("no such column").
DEFERRED_INDEX_SQL = """
...
class SessionDB:
    ...
    def _is_fts5_unavailable_error(exc: sqlite3.OperationalError) -> bool:
        err = str(exc).lower()
        # FTS5 模块缺失（老版本 SQLite 未编译 FTS5 扩展）
        if "no such module" in err and "fts5" in err:
            return True
        # FTS5 存在但缺少 trigram tokenizer（SQLite < 3.34.0 不支持 trigram）。
        # trigram 是 FTS5 的 tokenizer，缺失时应与 FTS5 不可用同等降级处理，
        # 否则 _ensure_fts_schema 会抛异常冒泡成 500（见 companion_api chat_endpoint）。
        if "no such tokenizer" in err and "trigram" in err:
            return True
        return False
